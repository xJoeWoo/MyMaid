...
public class Weibo_UserTimeLine extends Thread {
	...
	public Weibo_UserTimeLine(boolean isUid, String screenNameOfUid, int count, Handler handler){
		this.isUid = isUid;
...
//				+ "&count=" + count + "&trim_user=1");
		...
		if(!isUid)
		httpGet = new HttpGet(USER_TIMELINE_URL + "?access_token="
				+ WeiboConstant.ACCESS_TOKEN + "&screen_name=" + screenNameOfUid
				+ "&count=" + count);
		else 
			httpGet = new HttpGet(USER_TIMELINE_URL + "?access_token="
...
//					map.put(RETWEETED_STATUS_UID, statuses.get(i).getRetweetedStatus().getUser().getId());
